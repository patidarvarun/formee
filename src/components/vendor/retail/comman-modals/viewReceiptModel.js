...
class ReceiptModel extends React.Component {
  ...
  renderDescription = (oders) => {
    let orderData = oders.order_detail && Array.isArray(oders.order_detail) && oders.order_detail.length ? oders.order_detail : []
   ...
   */
  getTotal = (oders) => {
    let orderData = this.props.invoiceDetails.order_detail && Array.isArray(this.props.invoiceDetails.order_detail) && this.props.invoiceDetails.order_detail.length ? this.props.invoiceDetails.order_detail : []
    if(orderData && orderData.length){
   ...
   */
  render() {
    const { toUser,messageData,subTotal } = this.state
              ...
              <Row>
                <Col xs={24} sm={24} md={12} lg={12}>
                  <div className="left-block">
                    <b>RECIPIENT</b>
                    <p>{invoiceDetails.customer_fname} {invoiceDetails.customer_lname}</p>
                    <p>{invoiceDetails.customer_address1} {invoiceDetails.customer_address2}</p>
                    <p>{invoiceDetails.customer_postcode} {invoiceDetails.customer_city}  {invoiceDetails.customer_state}</p>
                    <p>VAT no.: 12345678</p>
                ...
                <div className="panel-body">
                  <div className="table-responsive">
                    <table className="table table-condensed">
                      ...
                      <tbody>
                        {this.renderDescription(this.props.invoiceDetails)}
                        <tr>
                          <td className="thick-line"></td>
                          <td className="thick-line"></td>
                          <td className="thick-line text-center gray top-bdr"><strong>Subtotal</strong></td>
                          <td className="thick-line text-right top-bdr">${this.props.invoiceDetails.order_subtotal}</td>
                        </tr>
                        <tr>
                          <td className="no-line"></td>
                          <td className="no-line"></td>
                          <td className="no-line text-center gray top-bdr"><strong>DISCOUNT</strong></td>
                          <td className="no-line text-right top-bdr">${this.props.invoiceDetails.order_discount}</td>
                        </tr>
                        <tr>
                          <td className="no-line"></td>
                          <td className="no-line"></td>
                          <td className="no-line text-center total top-bdr"><strong>Total</strong></td>
                          <td className="no-line text-right price top-bdr">{`$${this.props.invoiceDetails.order_grandtotal}`}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
