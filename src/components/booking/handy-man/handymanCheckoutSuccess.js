...
class handymanCheckoutSuccess extends React.Component {
  constructor(props) {
    console.log('props handymanCheckoutSuccess', props);
    super(props);
  ...
  componentDidMount() {
    console.log("this.props.checkoutData handymancheckout", this.props.checkoutData);
    ...
    if (this.props.checkoutData && this.props.location.search) {
      ...
      if (this.props.checkoutData) {
        ...
        this.props.eventjobCheckoutSuccess(
          ...
          (res) => {
            console.log(res,"eventjobCheckoutSuccess handycheckout");
  ...
  handymanAcceptBooking = () => {
    ...
    this.props.checkoutHandymanCustomerBooking(reqData, (res) => {
      ...
      //  toastr.success(langs.success, langs.messages.event_booking_success);
        ...
        //   pathname: `/`,
        // });
        if (this.state.selectedPaymentMethod === "stripe") {
         toastr.success(langs.success, langs.messages.event_booking_success);
  ...
  eventCheckout = () => {
    ...
    this.props.eventEnquiryCheckout(reqData, (res) => {
      this.props.disableLoading();
      if (res.status === 200 && res.data.success == true) {
        console.log('eventEnquiryCheckout@@@@@',res)
        if (this.state.selectedPaymentMethod === "stripe") {
          toastr.success(langs.success, langs.messages.event_booking_success);
          this.setState({ visible: true });
  ...
  membershipFitnessCheckout = () => {
    const { package_id, start_date, customer_name, customer_phone, phonecode } =
      this.props.location.state;
    ...
    this.props.getFitnessServiceBookingCheckout(
  ...
  render() {
    ...
    return (
      <Layout style={{ overflowX: "visible", background: "#fff" }}>
                ...
                <div>
                  <Button
                    ...
                    onClick={() => {
                      ...
                      if (selectedPaymentMethod == "saved card") {
                        this.onSucessPayment();
                      }
                    }}
                  >
        ...
        >
          <p>
            Your payment of {this.state.orderInfo.order_grandtotal} was
...
export default connect(mapStateToProps, {
  ...
  getBookingIdByEnquiryId,
  getTraderjobIdByQuoteId
})(withRouter(handymanCheckoutSuccess));
