...
class BookingCheckout extends React.Component {
  constructor(props) {
    //console.log("props BookingCheckout ", props);
    super(props);
    ...
    //console.log(this.state.invoiceDetails, "inv-detail")
    //console.log(this.state.traderDetailsResponse, "traderDetailsResponse")
    //console.log(this.props.checkoutData, "checkoutttttt")
  ...
  componentDidMount() {
      ...
      if (this.props.checkoutData && !query_temp.get("order_ids")) {
        ...
        this.props.eventjobCheckoutSuccess(
          ...
          (res) => {
            ...
            //console.log(res, "eventjobCheckoutSuccess 1");
          }
        );
      }
      if (this.props.checkoutData) {
    ...
    if (booking_type == "event") {
      ...
      //console.log('this.props.location.state.enquiryId', this.props.location.state);
  ...
  eventCheckout = () => {
    ...
    // const { enquiryBooking } = this.state.enquiryBooking;
    //console.log("enquiryBooking@@", this.state);
    //console.log("this.props.location.state@@", this.props.location.state);
    ...
    if (
      ...
      reqData.payment_source_id = this.state.stripePaymentGateWayResponse.id;
      // reqData.event_booking_id= this.state.enquiryBooking.id;
    }
  ...
  retailProductCheckout = () => {
    ...
    const { placeOrderReqData,selectedAddress } = this.props.location.state;
    ...
    placeOrderReqData.order.paymentMethod = this.state.selectedPaymentMethod
      ? this.state.selectedPaymentMethod
      : "stripe";
    placeOrderReqData.order.payment_method = placeOrderReqData.order.payment_method;
    ...
    placeOrderReqData.user_id = placeOrderReqData.order.user_id;
    placeOrderReqData.payment_method = placeOrderReqData.order.payment_method;
    placeOrderReqData.address_id = selectedAddress.id;
    placeOrderReqData.cart_classified_ids = placeOrderReqData.orderDetails[0].classified_id;

    // let finalPlaceOrderReqData = {
    //   user_id:placeOrderReqData.order.user_id,
    //   cart_classified_ids:placeOrderReqData.orderDetails[0].classified_id,
    //   payment_method:placeOrderReqData.order.payment_method,
    //   payment_source_id:placeOrderReqData.order.payment_source_id,
    //   address_id:selectedAddress.id
    // }

    if(placeOrderReqData.order.payment_method === "stripe"){
    ...
    if(placeOrderReqData.order.payment_method === "paypal"){
      this.props.placeOrderAPI(placeOrderReqData, (res) => {
      ...
      this.props.beautyServiceBookingCheckout(
        ...
        (res) => {
          ...
          if (res.status === 200 && res.data.success) {
            ...
            // });
            this.setState({ invoiceDetails: res.data })
            if (this.state.selectedPaymentMethod === "stripe" || this.state.selectedPaymentMethod === "saved card") {
              toastr.success(langs.success, langs.messages.service_booking_checkout_success);
              this.setState({ eventBookingPurchaseSuccessful: true });
  ...
  handleRetailCartCheckout = () => {
    ...
    let discountAmount =
      ...
      100;
    if (selectedPaymentMethod === "stripe" && stripePaymentGateWayResponse !== "") {
      placeOrderReqData.order.stripe_charge_id =
        stripePaymentGateWayResponse.id;
    }

    if (this.state.selectedPaymentMethod == "saved card") {
      placeOrderReqData.order.payment_source_id = this.state.selectedSavedCard[0].source_id;
      placeOrderReqData.order.payment_method = "stripe";
    }
    ...
    //placeOrderReqData.cart_classified_ids = parseInt(cart_classified_ids);
    ...
    this.props.placeOrderAPI(placeOrderReqData, (res) => {
      ...
      if (res.status === 200 && res.data.status == true  ) {
       ...
       this.setState({invoiceDetails: []})
        ...
        this.setState({ eventBookingPurchaseSuccessful: true, orderInfo: placeOrderReqData.order });
      } else {
        toastr.error(
          langs.error,
          langs.messages.service_booking_checkout_error,
  ...
  render() {
    ...
    //console.log(this.state.invoiceDetails, "iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii")
    const { amount, customer_name, booking_type } = this.props.location.state;
    const {orders} = this.state.orderData;
    ...
    return (
      <Layout style={{ overflowX: "visible", background: "#fff" }}>
        ...
        >
          <p>
            Your payment of {orderInfo && orderInfo.order_grandtotal} was
        ...
        <Modal
          title="Purchase Complete"
          visible={eventBookingPurchaseSuccessful}
          footer={false}
          onCancel={() => {
          ...
          <Button
            ...
            onClick={() => {
              ...
              this.setState({ receiptModalEventBooking: false }, () =>
                ...
                // this.props.history.push(`/payment-complete-orders?user_id=${this.propsloggedInUser.id}&order_id=${22}`);
                //  this.props.history.push(`/payment-complete-orders?user_id=${orders && orders.user_id}&order_id=${orders.order_id}`)
              // );
              // this.changeStep(3);
            }}
          >
        ...
        {receiptModalEventBooking && (
          <PDFInvoiceModal
            ...
            isViewInvoice={true}
            enquiryDetails={this.state.invoiceDetails}
            // orderData={this.state.orderData}
            // user_id={this.state.orderData && this.state.orderData.user_id}
