...
import { getBookingCatLandingRoute, getBookingMapDetailRoute, getBookingSubcategoryRoute, getBookingDetailPageRoute } from '../../../../common/getRoutes'
...
class ViewClasses extends React.Component {
    ...
    getFitnessClassListing = (profileId) => {
        ...
        this.props.getFitnessClassListing({ id: traderProfileId }, (res) => {
            ...
            if (res.data && res.data.status === 200) {
                ...
                traderClasses.unshift({ id: -1, class_name: 'All' });
                this.setState({
                ...
                if (cId) {
                    this.getScheduleData(cId, profileId)
                } else {
    ...
    */
    getDetails = () => {
        ...
        this.props.getBookingDetails(reqData, (res) => {
            ...
            if (res.status === 200) {
                ...
                this.getFitnessClassListing(res.data.data.trader_profile.id)
            }
        })
    }
     ...
     */
    getScheduleData = (e) => {
        const { classes, selectedDate, detail } = this.state;
        ...
        this.props.getFitnessClassSchedule({
            trader_profile_id: profileId,
            trader_class_id: e === -1 ? '' : e, date_from, date_to
        }, (res) => {
     ...
     */
    displayClasspBuyModal = (selectedSpaService, selectedClass) => {
    ...
    renderSchedule = (date) => {
        const { selectedSchedule, selectedClass, scheduleRoom, classes } = this.state;
        ...
        if (index < 0) {
            ...
            return selectedSchedule.map((el) => {
                if (el.day === String(day)) {
                    let i = classes.findIndex((c) => c.id === el.trader_class.id)
                    let className = classes[i].class_name
                    return (
                        <>
                            <Row gutter={[42, 0]} className="calender-detail-item">
                                ...
                                <Col md={12}>
                                    <div className="pt-5">
                                        <Text>
                                            <span className="table-detail-view-schu">{className}</span>  <br />
                                            <span className="pt-2 blue-text inline-block">{el.instructor_name} / Studio {scheduleRoom}</span>
                                        </Text>
                                    </div>
                                </Col>
                                <Col md={6} className="text-right">
                                    <Button className="yellow-btn" onClick={() => this.displayClasspBuyModal(el, classes[i])}> Book</Button>
                                </Col>
                            ...
                            <Col md={24}>
                                <Select
                                    ...
                                    onChange={(e) => {
                                        ...
                                        let i = classes.findIndex((c) => c.id === e)
                                        if (i >= 0) {
                                            this.getScheduleData(e)
                                        }
                                    }}
                                    className="w-100 select-box-view-sch"
