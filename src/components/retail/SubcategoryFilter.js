...
import { getRetailFilterRoute } from "../../common/getRoutes";
...
class GeneralClassifiedGeneral extends React.Component {
   ...
   */
  handleSearch = (value) => {
    const { selectedCity, searchLatLng, selectedOption,tabKey } = this.state;
  ...
  renderSearchOptions = () => {
    let parameter = this.props.match.params;
    let allData = parameter.all === langs.key.all ? true : false;
    let redirectUrl = getRetailFilterRoute(
    ...
    return (
      <Fragment>
        <div className="location-search-wrap11">
          <Tabs className="banner-tab" onChange={(key) => this.setState({tabKey:key })}>
