...
import { Button, Upload, message, Avatar, form, Row, Typography, Divider, Space, Spin } from 'antd';
import { LoadingOutlined, UserOutlined, LockOutlined,PlusOutlined } from '@ant-design/icons';
import {Checkbox,Select, Form, Input, Col } from 'antd';
import { langs } from '../../../../config/localization';
import { getTraderProfile, disableLoading, enableLoading, getUserProfile, saveTraderProfile, changeUserName, changeMobNo, logout, changeEmail, changeProfileImage, changeAddress, sendOtp } from '../../../../actions/index'
import { DEFAULT_IMAGE_TYPE } from '../../../../config/Config'
import { required, email, minLength, maxLength, validatePhoneNumber, validMobile } from '../../../../config/FormValidation'
...
class BasicDetail extends React.Component {
   ...
   componentDidMount() {
        ...
        userDetails.user.trader_service_images && userDetails.user.trader_service_images.map((el, index) => {
        ...
        this.setState({
            ...
            editorState1: trader_profile && trader_profile.description && BraftEditor.createEditorState(trader_profile.description),
            imageUrl: userDetails.image !== undefined ? userDetails.image : DEFAULT_IMAGE_TYPE,
            trader_working_hours: userDetails.user.trader_working_hours,
            is_public_closed: trader_profile && trader_profile.is_public_closed === 0 ? false : true
          })
        }
    }
    ...
    getInitialValue = () => {
        ...
        console.log('trader_profile: ***', userDetails);
        ...
        let temp = {
            ...
            fname: firstName,
            lname: lastName,
            description : trader_profile && BraftEditor.createEditorState(trader_profile.description),
    ...
    onFinish = (value) => {
        ...
        const { address,editorState1,is_public_closed,fileList } = this.state;
    ...
    renderOperatingHoursRow = (startTime, endTime, day, is_open) => {
        ...
        return (
          <div className="fr-op-hrs">
            <Form.Item
              ...
              value={startTime}
              rules={[{ required: (currentField && currentField[`operating_end_hours${rowIndex}`] !== undefined), message: 'This field is required.' }]}
            >
              <Select
                ...
                onChange={(e) => {
                  ...
                  if (e !== undefined && index >= 0) {
                    ...
                    currentField[`operating_start_hours${rowIndex}`] = start.format("hh:mm A");
              ...
              >
                {userDetails.time_slots &&
                  Object.keys(userDetails.time_slots).map(function (keyName, index) {
                    // 
                    return (
                      <Option value={keyName} key={userDetails.time_slots[keyName]}>{userDetails.time_slots[keyName]}</Option>
                    )
                  })}
              </Select>
            </Form.Item>
            <Form.Item
              rules={[{ required: (currentField && currentField[`operating_start_hours${rowIndex}`] !== undefined), message: 'This field is required.' }]}
              ...
              <Select
                ...
                onChange={(e) => {
                  ...
                  if (e !== undefined && index >= 0) {
                    ...
                    currentField[`operating_end_hours${rowIndex}`] = end.format("hh:mm A");
                    this.formRef.current.setFieldsValue({ ...currentField })
              ...
              >
                {userDetails.time_slots &&
                  Object.keys(userDetails.time_slots).map(function (keyName, index) {
                    ...
                    return (
                      <Option disabled={disabled} value={keyName} key={userDetails.time_slots[keyName]}>{userDetails.time_slots[keyName]}</Option>                                    // <Option>{userDetails.time_slots[keyName]}</Option>
                    )
                  })}
              </Select>
            </Form.Item>
      ...
      renderOperatingHours = () => {
        ...
        return Object.keys(userDetails.days).map(function (day, index) {
          let startTime = '';
          if (userDetails.user.trader_working_hours[index] && userDetails.user.trader_working_hours[index].start_time) {
            startTime = userDetails.user.trader_working_hours[index] && userDetails.time_slots[userDetails.user.trader_working_hours[index].start_time.substring(0, 5)]
            
          }
          let endTime = '';
          if (userDetails.user.trader_working_hours[index] && userDetails.user.trader_working_hours[index].end_time) {
            endTime = userDetails.user.trader_working_hours[index] && userDetails.time_slots[userDetails.user.trader_working_hours[index].end_time.substring(0, 5)]
          }
            ...
            <div className="operating-content-block">
              <Row gutter={30}>
                ...
                <Col xs={24} sm={12} md={24} lg={15} xl={20}>
                  {me.renderOperatingHoursRow(startTime
                    , endTime, day, userDetails.user.trader_working_hours[index].is_open)}
                </Col>
              </Row>
            </div>
          )
    ...
    render() {
        ...
        if(userDetails){
        const { trader_profile } = userDetails.user;
        const { loading,editorState1,fileList, address,is_public_closed} = this.state;
            ...
            <Fragment>
                <div className='upload-profile--box'>
                    <div className='upload-profile-pic'>
                        {userDetails.user.image ? <Avatar size={91} src={userDetails.user.image} /> : <Avatar size={91} icon={<UserOutlined />} />}
                    </div>
                    <div className='upload-profile-content'>
                        {<Title level={4}>{trader_profile && `${trader_profile.contact_name && converInUpperCase(trader_profile.contact_name)}`}</Title>}
                        <Text className='fs-11'>
                            {/* {this.props.userDetails.address} */}
                            (Member since -{ trader_profile && trader_profile.created_at && dateFormate(trader_profile.created_at)})
                            </Text>
                        <div className='mt-15'>
                            <Upload
                                name='avatar'
                        ...
                        <Row gutter={14} className="fr-name-block">
                            ...
                            <Col span={6} className="fr-name-leftblock" >
                                <div class="ant-col ant-form-item-label">
                                    <label className='ant-form-item-required'>Contact Name</label>
                                </div>
                                <Form.Item
                                    name='fname'
                                    rules={[required('First name')]}
                        ...
                        <Row gutter={28}>
                            <Col span={12}>
                                <Form.Item
                                    label='Address'
                                    name='address'
                                    rules={(address === '' || address == undefined || address === null) && [required('Address')]}
                                >
                                    <PlacesAutocomplete
                                        name='address'
...
export default connect(
    mapStateToProps,
    { getTraderProfile, disableLoading, getUserProfile, saveTraderProfile, logout, changeUserName, changeMobNo, changeEmail, changeProfileImage, changeAddress, sendOtp }
)(BasicDetail);