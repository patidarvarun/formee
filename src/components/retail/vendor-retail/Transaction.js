...
import { Layout, Card, Typography, Button, Tabs, Table, Row, Col, Input, Select } from 'antd';
import AppSidebar from '../../dashboard-sidebar/DashboardSidebar';
import history from '../../../common/History';
import { langs } from '../../../config/localization';
import Icon from '../../customIcons/customIcons';
import { getAllTransactionList, enableLoading, disableLoading } from '../../../actions'
...
class Transaction extends React.Component {
    ...
    */
     ...
     */
    render() {
        const { allTransaction, invoiceModel, invoiceDetails, search_keyword, catId } = this.state
        const { TabPane } = Tabs;
        ...
        const columns = [
                    ...
                    return (
                        <>
                            <div className='user-icon mr-13 d-flex userdetail-itemtable'>
                                <div>{row.category_name ? row.category_name : ''} <br />{row.sub_category_name ? row.sub_category_name : ''}</div>
                            </div>
                        </>
                    )
                }
            ...
            },
            {
                ...
                key: 'Price',
                render: (cell, row, index) => {
                    return <div className="price"> AU$${salaryNumberFormate(row.item_price)}</div>
                }
            },
            {
                title: 'Type',
            ...
            },
            {
                ...
                render: (cell, row, index) => {
                    let status = row.order_status === 'Delivered' || row.order_status === 'Shipped' || row.order_status === 'delivered' || row.order_status === 'shipped' ? 'Paid' : 'Unpaid'
                    return (
                        <div className="right-action">
                            <Row className='user-retail'>
                                <Col md={22}>
                                    <Button className={status === 'Paid' ? 'success-btn' : 'cancel-btn'}>
                                        {status}
                                    </Button>
                                </Col>
                            </Row>
            ...
            },
            {
                title: 'Views',
                render: (cell, row, index) => {
                    return <div> <a className="order" clhref="javascript:void(0)" onClick={() => this.setState({ invoiceDetails: row, invoiceModel: true })}>View</a></div>
                }
            },
        ]
        return (
                                ...
                                <div className='top-head-section'>
                                    ...
                                    <div className='right'>
                                        <div className='right-content'>
                                            <PostAdPermission title={merchant ? 'Start selling' : 'Post Ad'} />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className='profile-content-box'>
                                    ...
                                    >
                                        ...
                                        <Row className="grid-block">
                                            <Row style={{ height: '56px' }} className="w-100" align="middle" justify="space-between">
                                                <Col md={12}><h2 className="mb-0 pb-0">Transaction Management<span style={{ marginLeft: "25px" }}>{`(You have ${allTransaction.length} orders)`}</span></h2></Col>
                                                <Col md={12} >
                                            ...
                                            <Col md={24} >
                                                <Table dataSource={allTransaction} columns={columns} className="inspectiondetail-table retail-order-table"></Table>
                                            </Col>
                                        </Row>
                                    </Card>
                                </div>
...
export default connect(
    mapStateToProps,
    { getAllTransactionList, enableLoading, disableLoading }
)(Transaction)
