...
class VendorMyBookingsSpa extends React.Component {
  constructor(props) {
    super(props);
    let input = new Date();
  ...
  componentDidMount() {
    ...
    this.getVendorServiceSpaBookings(this.state.page);
  ...
  onFinishReview = (values) => {
    ...
    this.props.enableLoading();
    this.props.wellbeingServiceBookingsRating(requestData, (res) => {
  ...
  onTabChange = (key, type) => {
    ...
    if (key == "1") {
      this.getVendorServiceSpaBookings(1);
    } else {
      this.getVendorServiceSpaBookingsHistory(1);
    }
  };

  getVendorServiceSpaBookings = (page) => {
    ...
    this.props.listVendorServiceSpaBookings(reqData, (res) => {
      this.props.disableLoading();
      if (res.status === 200) {
        // this.setState({ vendorSpaBookingList: VENDOR_UPCOMING_BOOKING_LIST, totalRecordVendorSpaBooking: res.data.data.total_record ? res.data.data.total_record : 0 });
        this.setState({
          vendorSpaBookingList: res.data.data.service_bookings,
          totalRecordVendorSpaBooking: res.data.data.total_record
  ...
  handleBookingPageChange = (e) => {
    this.getVendorServiceSpaBookings(e);
  };

  getVendorServiceSpaBookingsHistory = (page) => {
    const { id, trader_profile_id } = this.props.loggedInUser;
    const {
      bookingListCalenderView,
      monthStart,
    ...
    this.props.listVendorServiceSpaBookingsHistory(reqData, (res) => {
      this.props.disableLoading();
      if (res.status === 200) {
        //this.setState({ vendorSpaBookingHistoryList: VENDOR_HISTORY_BOOKING_LIST, totalRecordVendorSpaBookingHistory: res.data.data.total_record ? res.data.data.total_record : 0 })
        this.setState({
          vendorSpaBookingHistoryList: res.data.data.service_bookings,
          totalRecordVendorSpaBookingHistory: res.data.data.total_record
            ? res.data.data.total_record
            : 0,
        });
      }
  ...
  handleHistoryBookingPageChange = (e) => {
    this.getVendorServiceSpaBookingsHistory(e);
  };
  ...
  renderVendorUpcomingSpaBooking = () => {
    const { vendorSpaBookingList } = this.state;
    if (vendorSpaBookingList && vendorSpaBookingList.length > 0) {
      return (
        <Fragment>
          {vendorSpaBookingList.map((value, i) => {
              ...
              <div
                ...
                onClick={() =>
                  this.props.history.push({
                    pathname: `/vendor-spa-bookings-detail/${value.id}`,
                  })
                }
                key={`vdr_spabookings_${i}`}
              >
                <Row gutter={0}>
                  ...
                  {/* <Link to={`/vendor-spa-bookings-detail/${value.id}`} className='blue-link'>Details</Link> */}
                </Row>
              </div>
            );
          })}
          <Pagination
            ...
            total={this.state.totalRecordVendorSpaBooking} //total number of card data available
  ...
  renderHistoryBooking = () => {
    const { vendorSpaBookingHistoryList } = this.state;
    if (vendorSpaBookingHistoryList && vendorSpaBookingHistoryList.length > 0) {
      return (
        <Fragment>
          {vendorSpaBookingHistoryList.map((value, i) => {
            return (
              <div
                className="my-new-order-block"
                onClick={(e) => {
                  e.stopPropagation();
                  this.props.history.push({
                    pathname: `/vendor-spa-booking-history-detail/${value.id}`,
                  });
                }}
                key={`vdr_spabookings_history_${i}`}
              >
                <Row gutter={0}>
                  <Col xs={24} sm={24} md={24} lg={18} xl={18}>
                    <div className="odr-no">
                  ...
                  {/* <Link to={`/vendor-spa-booking-history-detail/${value.id}`} className='blue-link'>Details</Link> */}
                </Row>
              </div>
            );
          })}
          <Pagination
            ...
            total={this.state.totalRecordVendorSpaBookingHistory} //total number of card data available
   ...
   */
  render() {
    const { selectedBookingDate, vendorCalenderBookingList, calenderView } =
      this.state;
    const { customerRating, showReviewModal } = this.state;
    const radioStyle = {
      display: "block",
      height: "30px",
      lineHeight: "30px",
                      ...
                      >
                        <Tabs onChange={this.onTabChange} defaultActiveKey="1">
                          <TabPane tab="Bookings" key="1">
                            <h3>
                              You have {this.state.totalRecordVendorSpaBooking}{" "}
                              appointments in total
                            </h3>
                            {this.renderVendorUpcomingSpaBooking()}
                          </TabPane>
                          <TabPane tab="History" key="2">
                            <h3>
                              You have{" "}
                              {this.state.totalRecordVendorSpaBookingHistory}{" "}
                              appointments in total
                            </h3>
