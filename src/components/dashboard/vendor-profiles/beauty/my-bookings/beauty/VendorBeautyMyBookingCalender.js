...
class VendorBeautyMyBookingCalender extends React.Component {
  ...
  getVendorWellBeingMonthBookingsCalenderCallback = (response) => {
    ...
    if (response.status === 200 && response.data) {
      ...
      bookingData.selectedDateData.length > 0 && bookingData.selectedDateData[0].bookings.length > 0 && bookingData.selectedDateData[0].bookings.map(item => {
        fullCalenderData.push({ title: `Appointment with ${item.customer.name}`, description: item.customer.name, start: `${item.booking_date} ${item.start_time}`, end: `${item.booking_date} ${item.end_time}` });
      });
      //
  ...
  render() {
    ...
    return (
      <Layout>
        <Layout>
          <AppSidebar activeTabKey={DASHBOARD_KEYS.BEAUTY_CALENDER} history={history} />
          <Layout>
                      ...
                      >
                        <div className="fm-vcalender-wrap">
                          <Row className="fm-top-title">
                            <Col xs={24} sm={24} md={24} lg={12} xl={12}><h5>{moment(selectedBookingDate).format("D MMM YYYY")}</h5></Col>
