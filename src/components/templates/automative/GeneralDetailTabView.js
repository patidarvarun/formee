...
import { Empty, Checkbox, Typography, Avatar, Tabs, Row, Col, Collapse } from 'antd';
import { convertHTMLToText, salaryNumberFormate } from '../../common';
import { getClassfiedCategoryDetail, enableLoading, disableLoading, openLoginModel } from '../../../actions/index'
...
class GeneralDetailTabView extends Component {
    ...
    */
    getDetails = (filterKey) => {
        ...
        this.props.getClassfiedCategoryDetail(reqData, (res) => {
            console.log('step2 detail ##', res.data.data);
    ...
    onTabChange = (key, type) => {
        this.setState({ activeTab: key, reviewTab: false, makeReviewtabOpen: false });
    };
   ...
   */
    renderSpecification = (data) => {
        let temp = data && Array.isArray(data) && data.length ? data.filter(el => el.key !== 'Features' &&  el.key !== 'Price' && el.slug !== 'is_price_negotiable?' && el.slug !== 'textArea') : []
        let sorted_list = temp && temp.sort((a, b) => {
        ...
        return sorted_list && Array.isArray(sorted_list) && sorted_list.map((el, i) => {
            ...
            return (
                <Fragment key={i}>
                    <Col span={4}  style={{marginBottom:"13px"}}><Text className='strong'>{el.key}</Text></Col>
                    <Col span={8}  style={{marginBottom:"13px"}}><Text>{value}</Text></Col>
                </Fragment>
            )
        })
    }
     ...
     */
    renderFeatures = (data) => {
        let temp = data && Array.isArray(data) && data.length && data.filter(el => el.key === 'Features')
        ...
        if (features && Array.isArray(features) && features.length) {
            ...
            return (
                <div className="data-error"><Empty description={'No Features Found for this ad'} /></div>
            )
        }
    }

    render() {
        const { tempSlug, setPriorityTab, allData, classified_id, cat_id, similarAd, isLoggedIn, makeReviewtabOpen } = this.props;
        const { activeTab, classifiedDetail } = this.state;
        let cat_name = classifiedDetail && classifiedDetail.categoriesname && classifiedDetail.categoriesname.name 
        console.log(classifiedDetail && classifiedDetail,'classifiedDetail',cat_name)
        return (<Tabs type='card' className={'tab-style3 product-tabs'}
            activeKey={activeTab}
            onChange={(e) => {
                console.log('e: $$', e);
                this.onTabChange(e)
        ...
        >
            <TabPane tab='Details' key='1'>
                <Row gutter={[0, 0]}>
                    <Col md={18}>
                        ...
                        <Row className='pt-5'>
                        {cat_name === 'Automotive' && allData && allData.spicification && this.renderSpecification(allData.spicification)}
                        </Row>
                        </div>
                    </Col>
                    <Col md={1}>&nbsp;</Col>
            ...
            {cat_name === 'Automotive' && <TabPane tab='Features' key='2'>
                <div className="feture-listing">
                        <h2>Standard Features</h2>  
                <Row gutter={[0, 0]}>
                    {allData && allData.spicification && this.renderFeatures(allData.spicification)}
                </Row>
                </div>
            </TabPane>}
                        ...
                        <div className='reviews-content-left'>
                            ...
                            <div className='reviews-content-avatar-detail'>
                                ...
                                <a className='fs-10 underline'>
                                    {isLoggedIn ? <Link to={`/user-ads/${tempSlug}/${cat_id}/${classified_id}`}>{`Found ${classifiedDetail.usercount} Ads`}</Link> :
...
export default connect(mapStateToProps, { getClassfiedCategoryDetail, openLoginModel, enableLoading, disableLoading })(React.memo(withRouter(GeneralDetailTabView)));
