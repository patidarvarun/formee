...
import { Empty, Divider, Checkbox, Menu, Tooltip, Layout, Typography, Tabs, Row, Col, Input, Select, Button, Rate, Collapse, Dropdown } from 'antd';
import Icon from '../../../components/customIcons/customIcons';
import {ExclamationCircleOutlined } from '@ant-design/icons';
import { getClassfiedCategoryDetail } from '../../../actions/classifieds';
import { enableLoading, disableLoading, addToWishList, removeToWishList, openLoginModel, getChildCategory } from '../../../actions/index'
...
class DetailPage extends React.PureComponent {
  ...
  */
  render() {
    const { isLoggedIn, loggedInDetail } = this.props;
    const {reportReviewModel, catId, categoryName, tempSlug, subCatId, subCatName, is_favourite, visible, makeOffer, showNumber, reviewTab, classifiedDetail, activeTab, allData, subCategory, reportAdModel, similarAd, makeReviewtabOpen, priorityTab } = this.state;
    ...
    let specification = allData && allData.spicification && Array.isArray(allData.spicification) && allData.spicification.length ? allData.spicification.filter(el => el.slug === 'is_price_negotiable?') : []
    ...
    let conditionlabel = categoryName === 'Automotive' || categoryName === 'Electronics' || categoryName === 'Books & Music' ? 'Condition' : 'Category'
    let conditionValue = categoryName === 'Automotive' || categoryName === 'Electronics' || categoryName === 'Books & Music' ? classifiedDetail.ad_type ? classifiedDetail.ad_type : 'New' : classifiedDetail.subcategoriesname && classifiedDetail.subcategoriesname.name
    return (
                    ...
                    <Row gutter={[0, 0]}>
                      ...
                      <Col span={8}>
                        ...
                        {classifiedDetail.classified_image && <Carousel className="mb-4" classifiedDetail={classifiedDetail}
                          slides={classifiedDetail.classified_image} parameter={parameter}
                        />}
                        {/* <Row gutter={[20, 0]}>
                        ...
                        <div className='product-detail-right'>
                          ...
                          <div className="left-block">
                            <table style={{width:"100%"}}>
                              <tr>
                                ...
                                <td><Title level={2} className='price'>
                                  {classifiedDetail.price ? `AU$${salaryNumberFormate(classifiedDetail.price)}` : 'Free'}
                                  {classifiedDetail.is_ad_free ? 'Free' : ''}
                                </Title>
                              ...
                              <tr>
                                <td><label>{conditionlabel}:</label></td>
                                <td className="text-detail">{conditionValue}</td>
                              </tr>
                              <tr>
                          ...
                          {isButtonVisible && <div className='action-btn-block'>
                             <Button
                              ...
                              {classifiedDetail.price !==0 && classifiedDetail.is_ad_free !==1 && <Button
                                type='default'
                          ...
                          <div className="add-review-block">
                            ...
                            <div className="add-no-right">
                              <Link to={categoryPagePath}>
                                ...
                                <div className="ad-num"><Paragraph className='text-gray mb-0'>AD No. {classified_id}</Paragraph></div>
                              </div>
                            </div>
...
export default connect(
  mapStateToProps,
  { getClassfiedCategoryDetail, addToWishList, removeToWishList, openLoginModel, enableLoading, disableLoading, getChildCategory }
)(DetailPage);