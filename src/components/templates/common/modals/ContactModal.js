...
class ContactModal extends React.Component {
  ...
  onFinish = (values) => {
    ...
    console.log(classifiedDetail, "receiverId", receiverId, classifiedid);
   ...
   */
  render() {
    const { flag, visible, classifiedDetail, userDetails, contactType } =
      this.props;
    ...
    return (
      <Modal
        title={
          contactType === "realstate" ? "Send Enquiry" : "Contact to advertiser"
        }
        visible={visible}
        className={"custom-modal style1 custom-modal-contactmodal-style"}
        footer={false}
        datata={classifiedDetail}
            ...
            {contactType !== "realstate" && (
              <Col md={6} className="align-right" offset={1}>
                <Text className="text-gray">
                  ...
                  {classifiedDetail.classified_users &&
                    `(Member since ${
                      classifiedDetail.classified_users.member_since
                        ? classifiedDetail.classified_users.member_since
                        : dateFormat4(
                            classifiedDetail.classified_users.created_at
                          )
                    })`}
                </Text>
              </Col>
