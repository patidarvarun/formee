  ...
  */
  ...
  handleDropdownchange = (value) => {
    ...
    if(value === 655){
      ...
      this.setState({isFloated: true, salaryMin: this.state.salaryMin})
    }
    if (flag) {
        const requestData = {
  ...
  renderSelect = (data,label) => {
    ...
    let min = label === 'Minimum Year' || label === 'Odometer From'
    let max = label === 'Maximum Year' || label === 'Odometer To'
        ...
        if(data.have_children && data.attr_type_name === 'Drop-Down'){
          return (
            ...
            <div className="input-grp-compact">
              <Row gutter={0}>
                <Col md={12}>
                  {this.renderSelect(data, data.att_name)}
                </Col>
                <Col md={12}>
  ...
  renderMaxPrice = () => {
    ...
    return (
      ...
      >
        ...
        <Select dropdownMatchSelectWidth={false} placeholder="Price Max." allowClear  onChange={(el) => this.setState({selectedMaxPrice:el})}>
            ...
            <Form.Item className="classifies-moreoptn" >
              ...
              {isMoreOption &&
                <div >
                  ...
                  {template !== 'job' && (slug === 'cars-vans' || slug === 'motorcycle' || slug === 'caravan-campers') ? <Col md={6}>
                      <div className="input-grp-compact">
                        <Row gutter={0}>
                          <Col md={12} >
                            {this.renderMinPrice()}
                    ...
                    <div className={currentField['listed_date']  ? "floating-label" : ''}>
                      <Form.Item
                        ...
                        label='Listed'
                      >
                        <Select placeholder='Listed' allowClear  onChange={() => this.setState({isFloated:true})}>
                    ...
                    {template === 'job' &&<Col md={6} >
                      ...
                      <Form.Item
                        ...
                        label="Salary Max"
                      >
                        <Select  dropdownMatchSelectWidth={false} className="right-selct-list" placeholder="Salary Max" allowClear onChange={(el) => this.setState({selectedMaxPrice:el})}>
                    ...
                    <div className={currentField['price_max']  ? "floating-label" : ''}>
                      <Form.Item
                        ...
                        label="Salary Max"
                      >
                        <Select placeholder="Salary Max" allowClear onChange={(el) => this.setState({selectedMaxPrice:el})}>
                          {this.state.salaryMin.map((el, i) => {
                            let disable = false;
                            if (
                              this.state.selectedMinPrice &&
