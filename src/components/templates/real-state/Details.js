...
import { Card, Empty, Menu, Tooltip, Layout, Typography, Avatar, Tabs, Row, Col, Breadcrumb, Form, Input, Select, Checkbox, Button, Rate, Modal, Dropdown, Divider } from 'antd';
import Icon from '../../../components/customIcons/customIcons';
import { UserOutlined, ExclamationCircleOutlined, ClockCircleOutlined, CheckOutlined } from '@ant-design/icons';
import { getChildCategory, enableLoading, disableLoading, addToWishList, removeToWishList, openLoginModel, getClassfiedCategoryDetail } from '../../../actions/index'
...
import { displayDateTimeFormate, convertHTMLToText, displayInspectionDate, formateTime, salaryNumberFormate } from '../../common';
...
import {convertISOToUtcDateformate, capitalizeFirstLetter } from '../../common'
...
class DetailPage extends React.Component {
    ...
    this.props.getClassfiedCategoryDetail(reqData, (res) => {
      ...
      if (res.status === 200) {
        ...
        this.setState({
          ...
          categoryName: res.data.category_name ? res.data.category_name : data && data.categoriesname ? data.categoriesname.name : '',
          subCatName: res.data.sub_category_name ? res.data.sub_category_name : data && data.subcategoriesname ? data.subcategoriesname.name : '',
          catId: res.data.category_id ? res.data.category_id : data && data.categoriesname ? data.categoriesname.id : '',
          tempSlug: res.data.template_slug ? res.data.template_slug : 'general',
          subCatId: res.data.sub_category_id ? res.data.sub_category_id : data && data.subcategoriesname ? data.subcategoriesname.id : '',
        })
      }
    })
  }
  ...
  */
  ...
  renderSpecification = (data) => {
    if(data && Array.isArray(data) && data.length){
      let sorted_list = data && data.sort((a, b) => {
      ...
      return sorted_list && Array.isArray(sorted_list) && sorted_list.map((el, i) => {
        return (
  ...
  */
  renderIcon = (data) => {
    const iconData = data.filter(el => el.key === 'Bedrooms' || el.key === 'Bathrooms' || el.key === 'Parking Type' || el.key === 'Shower' || el.key === 'Parking' || el.key === 'Property Type')
    return iconData && Array.isArray(iconData) && iconData.map((el, i) => {
      return (
        <li>
          ...
          {(el.key === 'Shower' || el.key === 'Bathrooms') && <img src={require('../../../assets/images/icons/bathroom.svg')} alt='' />}
          {(el.key === 'Shower' || el.key === 'Bathrooms') && <Text>{el.value}</Text>}
          {(el.key === 'Parking' || el.key === 'Parking Type') && <img src={require('../../../assets/images/icons/carpark.svg')} alt='' />}
          {(el.key === 'Parking' || el.key === 'Parking Type') && <Text>{el.value}</Text>}
    ...
    return item && Array.isArray(item) && item.length && item.map((el, i) => {
      return (
        <Row gutter={15} >
          <Col span={21}>
            <div className='inspection-list'>
              ...
              <Text className='ml-15'>
                {displayInspectionDate(new Date(el.inspection_date).toISOString())}
              </Text>
              <div className='right'>
                <Text>
          ...
          {visible && <Col span={3}  style={{ display:"flex", alignItems:"center" }}>
            <Button htmlType={'button'} type='primary' onClick={() => this.makeOfferModal(el)} >
              <img
  ...
  */
  renderFeatures = (features) => {
    if (features && Array.isArray(features) && features.length) {
      return features && features.map((el, i) => {
        return (
          <Col span={6} key={i}>
  ...
  renderRealestateFloorPlan = (data) => {
    const iconData = data.filter(el => el.slug === 'bedroom' || el.slug === 'type-bathroom' || el.slug === 'Parking Type' || el.slug === 'Shower' || el.slug === 'Parking' || el.slug === 'Property Type' || el.slug === 'car_spaces' || el.slug === 'Area Size' || el.slug === 'type_of_parking' || el.slug === 'Land Size' || el.slug === 'Floor Size' || el.slug === 'tenture_type'  || el.slug === 'furnished' || el.slug === 'available_from')
    return iconData && Array.isArray(iconData) && iconData.map((el, i) => {
      return (
        <Row key={i} >
          <Col span={15}><Text className='strong'>{el.key}</Text></Col>
          <Col span={9}><Text>{el.value}</Text></Col>
        </Row>
      )
    })
  }
  ...
  */
  render() {
    const { isLoggedIn, loggedInDetail } = this.props;
    const {imageModel,selectedInspection,flag, subCategory, catId, tempSlug, categoryName, subCatId, subCatName, isOpen, is_favourite, visible, classifiedDetail, reviewModel, allData, reportAdModel } = this.state;
    ...
    const dateTime = (
      <ul className='c-dropdown-content'>
        {inspectionTime && Array.isArray(inspectionTime) && inspectionTime.length && inspectionTime.map((el, i) =>
          <li key={i}  >
            { }
            <Text className={moment(el.inspection_date).format('YYYY-MM-DD') === currentDate ? 'active-date' : ''} >{displayInspectionDate(new Date(el.inspection_date).toISOString())}</Text>
            <Text className={moment(el.inspection_date).format('YYYY-MM-DD') === currentDate ? 'pull-right active-date' : 'pull-right'} >{formateTime(el.inspection_start_time)} - {formateTime(el.inspection_end_time)}</Text>
          </li>
        )}
      </ul>
    )
    ...
    let specification = allData && Array.isArray(allData.spicification) && allData.spicification.length ? allData.spicification.filter(el => el.slug === 'property_type' || el.slug === 'is_price_negotiable?') : []
    let propert_type = specification.length ? specification[0].value : ''
    let is_negotiable = specification && Array.isArray(specification) && specification.length ? specification[0].value : ''
    let temp = allData.spicification && Array.isArray(allData.spicification) && allData.spicification.length && allData.spicification.filter(el => el.key === 'Features')
    let features = temp && temp.length ? temp[0].value && temp[0].value.split(',') : []
    const number = (
      <Menu>
        <Menu.Item key='0'>
          {isLoggedIn ?
            <span className='' >
              {classifiedDetail.classified_users && <Tooltip placement='bottomRight' >
              <div><b><span>Contact {classifiedDetail.classified_users.name}</span></b>
                  <span> {classifiedDetail.contact_mobile ? classifiedDetail.contact_mobile :
                  ...
                  <div className='detail-page right-content-block'>
                    <Row gutter={[0, 0]}>
                      <Col span={8}>
                        ...
                        {classifiedDetail.classified_image && <Carousel className="mb-4" classifiedDetail={classifiedDetail}
                      ...
                      <Col span={16}>
                        <div className='product-detail-right'>
                          <div className="product-title-block">
                            ...
                            <div className="total-view"><Icon icon='view' size='20' /> <Text>{classifiedDetail.count} Views</Text></div>
                            </div>
                          ...
                          <div className="left-block">
                            <table style={{width:"100%"}}>
                              <tr>
                                ...
                                <td>
                                  <Title level={2} className='price'>{'AU$'}{salaryNumberFormate(classifiedDetail.price)}</Title>
                                  {is_negotiable && <Text className='mr-7 price-subtitle'>{is_negotiable ? 'Negotiable' : ''}</Text>}
                                </td>
                              </tr>  
                              <tr>
                                <td colspan="2" className="space-block">&nbsp;</td>                
                              </tr>
                              <tr>
                                <td><label>Date Listed:</label></td>
                                <td className="text-detail">{convertISOToUtcDateformate(classifiedDetail.created_at)}</td>
                              </tr>
                              <tr>
                                <td><label>Property Type:</label></td>
                                <td className="text-detail">{propert_type}</td>
                              </tr>
                              <tr>
                                <td><label>Location:</label></td>
                                <td className="text-detail">{classifiedDetail.location}</td>
                              </tr>
                              <tr>
                                <td colSpan="2">
                          ...
                          <div className="right-block">
                          {isButtonVisible && <div className='action-btn-block'>
                            <Button
                                type='default'
                              ...
                              {console.log('classifiedDetail.inspection_type', classifiedDetail.inspection_type)}
                              {inspectionTime && inspectionTime.length !== 0 && classifiedDetail.inspection_type !== 'By Appointment' && 
                                <Button
                                  type='default'
                                  onClick={() => this.makeOfferModal('')}
                          ...
                          <div className="add-review-block">
                            ...
                            <div className="review-start">
                              ...
                              <div className="review-text">
                                {classifiedDetail.classified_hm_reviews && classifiedDetail.classified_hm_reviews.length !== 0 && <span className='blue-link'>
                            ...
                            <div className="add-no-right">
                                ...
                                <div className="ad-num"><Paragraph className='text-gray'>AD No. {classified_id}</Paragraph></div>
                              </div>
                            </div>                              
                            </div>
                            ...
                            <div className="view-map testing-content change-log">
                                {classifiedDetail.subcategoriesname &&
                                  <p onClick={() => {
                                    ...
                                    className='blue-p'><ExclamationCircleOutlined /> Report this Ad</p>
                                }
                              </div>
                            </div>
                          </div>
                    ...
                    <Tabs type='card' className={'tab-style3 product-tabs'}>
                      ...
                      <div className="content-detail-block">
                            ...
                            {features && features.length !==0 && <Title level={4} className="block-heading-two">Property Features</Title>}
                                               <div className="feture-listing"> 
                <Row gutter={[0, 0]}>
                {features && features.length !==0 && this.renderFeatures(features)}
                </Row>
                </div>
                            </div>
                          </Col>
                    ...
                    <div className="floor-plan-img" title="Click to view floor plan">
                      {<img 
                        alt="example" 
                        src={classifiedDetail.floor_plan ? `${classifiedDetail.floor_plan}` : require('../../../assets/images/floor-plan.jpg')} 
                        onClick={() => this.setState({imageModel: true})}
                      />}
                    </div>
              <div className="floor-detail">
              {allData && allData.spicification && this.renderRealestateFloorPlan(allData.spicification)}
              </div>
                    </div>
                      ...
                      <TabPane tab='Inspection' key='3' className="book-inspection">
                        {inspectionData && classifiedDetail.inspection_type !== 'By Appointment' && <div>
                          <Title level={4}>{'Book an Inspection Time'}</Title>
                          <div className={'mt-20 mb-30'}>
                            {inspectionTime && inspectionTime.length !== 0 && this.renderInspectionTime(inspectionTime, isButtonVisible)}
                          </div>
                          {/* <Text>
                            Submit to secure your booking and be notified of any changes.<br />
                            Additional open for inspection times are available on request.
                          </Text> */}
                        </div>}
                        {inspectionData && classifiedDetail.inspection_type === 'By Appointment' &&
                          <Title level={4}>{'By Appointment'}</Title>}
                        {inspectionData === '' &&
                          <div className="inspection-no-data">
                              <ClockCircleOutlined className="clock"/>
                            {/* <Empty description={'The agent has not scheduled any inspections for this property'} /> */}
                            <div className="empty-discrip">The agent has not scheduled any inspections for this property</div>
                            <Button
                              type='default'
                            ...
                            <div className='reviews-content-left'>
                              ...
                              <div className='reviews-content-avatar-detail'>
                                ...
                                <a className='fs-10 underline'>
                                  {isLoggedIn ? <Link to={`/user-ads/${'realstate'}/${cat_id}/${classified_id}`}>{`Found ${classifiedDetail.usercount} Ads`}</Link> :
...
export default connect(
  mapStateToProps,
  { getChildCategory, getClassfiedCategoryDetail, addToWishList, removeToWishList, openLoginModel, enableLoading, disableLoading }
)(DetailPage);