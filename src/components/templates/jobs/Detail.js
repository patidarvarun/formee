...
import {Menu, Tooltip, Dropdown, Layout, Typography, Avatar, List, Tabs, Row, Col, Input, Button, Upload, message, Rate, Modal, Divider, Popover, Space } from 'antd';
import Icon from '../../../components/customIcons/customIcons';
import { UserOutlined, ExclamationCircleOutlined } from '@ant-design/icons';
import { getClassfiedCategoryDetail, getJobQuestions } from '../../../actions/classifieds';
import { getChildCategory, enableLoading, disableLoading, addToWishList, removeToWishList, openLoginModel } from '../../../actions/index'
...
class DetailPage extends React.Component {
    ...
    */
    ...
    render() {
        ...
        const { categoryName, tempSlug, subCatName, subCategory, subCatId, is_favourite, jobApplication, visible, responsbility, classifiedDetail, allData, salary, salary_type, company_name, about_job, opportunity, apply, about_you, btnDisable, reportAdModel } = this.state;
        ...
        let totalSalary = range1 && range2 ? `$${range1} - $${range2}` : range1 && `$${range1}`
        const menu = (
            <SocialShare {...this.props} />
        )
        let subCategoryPagePath = classifiedDetail.categoriesname ? getClassifiedSubcategoryRoute(TEMPLATE.JOB, categoryName, classifiedDetail.categoriesname.id, subCatName, classifiedDetail.subcategoriesname.id) : ''
        ...
        let temp = allData.spicification && Array.isArray(allData.spicification) && allData.spicification.length ? allData.spicification : []
        ...
        let image = classifiedDetail.classified_image && Array.isArray(classifiedDetail.classified_image) && classifiedDetail.classified_image.length ? classifiedDetail.classified_image[0] : '' 
              ...
              <Menu.Item key='0'>
                {isLoggedIn ?
                  ...
                  <span>
                    ...
                    <div>Please <span className='blue-link ml-16 fs-16' onClick={() => this.props.openLoginModel()} >Login</span><br/>to view number </div>
                  </span>}
              </Menu.Item>
            </Menu>
          )
                        ...
                        <Layout>
                            <Layout  style={{ width: 'calc(100% - 0px)', overflowX: 'visible' }}>
                                <Layout>
                                    ...
                                    <Row gutter={15} >
                                      <Col md={24} >
                                      <div className="month-detail category-name">
                            ...
                            to={subCategoryPagePath}>
                              ...
                              >
                                ...
                                />
                    {classifiedDetail.subcategoriesname &&  capitalizeFirstLetter(classifiedDetail.subcategoriesname.name)}
                              </Button>
                            </Link>}
                                    ...
                                    <Col md={3}>
                                        ...
                                        <div className="job-logo-img">
                                            <img
                                            src={classifiedDetail.company_logo ? classifiedDetail.company_logo : DEFAULT_IMAGE_CARD}
                                            onError={(e) => {
                                    ...
                                    <Col md={1}> &nbsp;</Col>
                                        <Col md={20}>
                                          <div className="job-title-button-detail">
                                              ...
                                              <Title level={2} className='title'>{classifiedDetail.title && capitalizeFirstLetter(classifiedDetail.title)}</Title>
                                                    ...
                                                    <div className='info'>
                                                      ...
                                                      <div className='info-detail'>
                                                            {classifiedDetail.created_at && <Text><span className="label">Date Listed:</span><span className="dark-color">{classifiedDetail && convertISOToUtcDateformate(classifiedDetail.created_at)}</span></Text>}
                                                        </div>
                                                        <div className='info-detail'>
                                                            {about_job && <Text><span className="label">Job Type:</span><span className="dark-color">{about_job}</span></Text>}
                                                        </div>
                                                      </div>
                                                      <div className="right-block">
                                                      <div className='rate-section info-detail'>
                                                      <span className="label">Reviews:</span> <span className="text-gray">{rate ? rate : 'No reviews yet'}</span>
                                                            {rate && <Rate disabled defaultValue={rate && rate ? rate : ''} />}
                                                        </div>
                                                        <div className='info-detail ad-details'>
                                                            {categoryName && <Text><span className="label">Ad Details:</span>
                                                            <Link to={categoryPagePath}><span className="light-btn">{categoryName}</span></Link></Text>}
                                              ...
                                              <div className="parent-right-block">
                                                <div className="ant-card-actions">
                                                  ...
                                                  <div className="total-view">
                                                          <Icon icon='view' size='20' /> <Text>{classifiedDetail.count} Views</Text>
                                                          </div>
                                                  </div>
                                                  <div className="right-block">
                                                  <ul>
                                              ...
                                              {isButtonVisible && <div className='right-content'>
                                        ...
                                        <Tabs type='card' className={'tab-style3 job-detail-content mt-0'}>
                                            ...
                                            <TabPane tab={(<span className='border-line'>Advertiser Information</span>)} key='4'>
                                                <Row className='reviews-content'>
                                                    <Col md={14}>
                                                            ...
                                                            <Title level={4} className='mt-0 mb-4'>
                                                                ...
                                                                <div className='reviews-rating'>
                                                                    <div className='product-ratting'>
                                                                        ...
                                                                        {rate && <Rate allowHalf defaultValue={rate && rate ? rate : 0} className='ml-6 mr-6' style={{ position: 'relative', top: '-1px' }} />}
                                                                        <Text className="text-gray">{rate ? `${rate} of 5.0 /` : ''}  {ratingLabel(rate)}</Text>
                                                                    </div>
                                                                </div>
...
export default connect(
    mapStateToProps,
    { getChildCategory, getClassfiedCategoryDetail, getJobQuestions, addToWishList, removeToWishList, openLoginModel, enableLoading, disableLoading }
)(DetailPage);