...
//import GeneralSearch from '../GeneralClassifiedNewSearch';
...
class SimpleLandingPage extends React.Component {
    ...
    */
    ...
    getAllData = () => {
        ...
        this.props.papularSearch({ module_type: langs.key.classified, category_id: cat_id }, res => {
            ...
            if (res.status === 200) {
                if (Array.isArray(res.data.success.data)) {
                    this.setState({ popularSearches: res.data.success.data })
                }
            }
        })
    ...
    getPopularSearchCities = () => {
        ...
        this.props.getPopularSearchCitiesOptions(reqData, (res) => {
            if (res.status === 200) {
                ...
                this.setState({ popularCities: [{ cityName: 'Melbourne' }, ...res.data.success.cities] })
            }
        })
    }
    ...
    */
    getMostRecentData = (id) => {
        ...
        //     this.props.disableLoading()
        //     if (res.status === 200) {
        //         this.setState({ mostRecent: res.data.data })
        //     }
        // })
            ...
            <Menu>
                {filteredCityList.map((city, index) =>
                    <Menu.Item key={index} onClick={() => this.props.getPopularSearchByCity({
                        ...
                        if (res.status === 200 && Array.isArray(res.data.success.searches)) {
                            
                            this.setState({ popularSearches: res.data.success.searches, selectedCity: city })
                        }
                    })}>
                        {city.cityName}
                    </Menu.Item>
                                ...
                                <Form name='location-form' className='location-search' layout={'inline'}>
                                    ...
                                    {/* {!this.state.isMoreOption &&
                                        <Row gutter={[8, 8]} className='location-more-form'>
                                            <Col md={12}>
                                                <Select placeholder='Any classification'>
                                                    <Option value='Any classification'>Any classification</Option>
                                                </Select>
                                            </Col>
                                            <Col md={12}>
                                                <Select placeholder='Job types'>
                                                    <Option value='Job types'>Job types</Option>
                                                </Select>
                                            </Col>
                                            <Col md={12}>
                                                <Select placeholder='Salary Estimate per hour'>
                                                    <Option value='Salary Estimate per hour'>Salary Estimate per hour</Option>
                                                </Select>
                                            </Col>
                                            <Col md={12}>
                                                <Select placeholder='Salary Estimate per year'>
                                                    <Option value='Salary Estimate per year'>Salary Estimate per year</Option>
                                                </Select>
                                            </Col>
                                            <Col md={12}>
                                            ...
                                            <Col md={12}>
                                                <Select placeholder='Listed date'>
                                                    <Option value='Listed date'>Listed date</Option>
                                                </Select>
                                            </Col>
                                        </Row>
                                    } */}
                                    {/* <div className='location-more-option'>
                                        <a onClick={this.toggleMoreOption.bind(this)} className={!this.state.isMoreOption && 'active'}>
                                            {this.state.isMoreOption ? 'More option' : 'Less'}
                                        </a>
                                    </div> */}
                                    <div className='location-more-option' >
