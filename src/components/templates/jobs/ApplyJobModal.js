...
import './resume-builder/resume.less'
...
*/
      ...
      return (
        <div className='resume-preview'>
          <div className='files-item'>
            <span>{file.name}</span>{` ( ${fileInKb} KB - Added about ${displayDateTimeFormate(today)} )`}
  ...
  */
    ...
    return experience.length && experience.map((el) => (
      <Row>
        ...
        <Col span={5} className='align-right'>
          <Text className='strong'>{`${this.blanckCheck(el.start_month)} ${this.blanckCheck(el.start_year)}  ${this.blanckCheck(el.complete_month, true)} ${this.blanckCheck(el.complete_year)}`}</Text>
        </Col>
      </Row>
    ))
  }
  ...
  renderEducation = (education) => {
    return education.length && education.map((el) => (
      <Row>
        ...
        <Col span={5} className='align-right'>
          <Text className='strong'>{`${this.blanckCheck(el.finished_from_month)} ${this.blanckCheck(el.finished_to_month)} ${this.blanckCheck(el.finished_to_month, true)} ${this.blanckCheck(el.finished_to_year)}`}</Text>
        </Col>
      </Row>
    ))
  }
  ...
  renderQuestions() {
    ...
    // [{ 'ans_value': 'I am a good person', 'qus_id': 22 }]
    ...
    return questions.map((el, i) => {
      ...
      return (<div>
        <Row className='mb-24'>
          ...
          <Col xs={24} sm={24} md={24} lg={13} xl={13} offset={1} >
            <div className="question-tag mb-9">{el.question}?</div>
            <div className="question-type  custon-question-type">
              {el.ans_type === QUESTION_TYPES.TEXT &&
                ...
                >
                  <Input className="shadow-input " onChange={(e) => {
   ...
   */
  render() {
    const { visible, classifiedDetail, loggedInDetail, userDetails, companyName, classified_hasmany_questions } = this.props;
    const { jobApplication, disableSendBtn, answer, resumeDetails, resumeFileList, coverLatterList, hideFirstModal } = this.state
    ...
    return (
      <div>
        ...
        >
          <div className='padding'>
            ...
            >
              ...
              <Form.Item
                label={<span>Resume<br /><span className='text-gray f-10'>Attaching a resume is optional</span></span>}
                name='resume'
              >
                ...
                {!appliedWithFormeeResume && <Row>
                  <Col span={18}>{this.renderFiles(resumeFileList, 'resume')}</Col>
                </Row>}
              ...
              >
                ...
                <Row>
                  <Col span={18}>{this.renderFiles(coverLatterList, 'cover_letter')}</Col>
                </Row>
              </Form.Item>
        ...
        {/* {jobApplication &&
                    <JobApplicationModal
                        jobVisible={jobApplication}
                        onJobCancel={() => this.setState({ jobApplication: false })}
