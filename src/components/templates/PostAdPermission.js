        ...
        if (!isLoggedIn) {
            ...
            this.props.checkPermissionForPostAd({ user_id: loggedInDetail.id }, res => {
                ...
                if (res.status === 200) {
                    ...
                    if (data.seller_type === langs.key.private) {
                    ...
                    } else if (data.seller_type === langs.key.business || data.seller_type === langs.key.merchant) {
                        ...
                        if (isjobPost) {
                            ...
                            if (limit) {
                                ...
                                this.setState({ permission: false })
                                toastr.warning(langs.warning, MESSAGES.POST_LIMIT)
                            } else if(data.seller_type === langs.key.merchant && data.is_stripe_accepted === 0 && data.is_paypal_accepted === 0) {
                                console.log('case2', )
                                this.props.history.push({
                                    pathname:'/payment',
                                    state: { pathname: pathName }
